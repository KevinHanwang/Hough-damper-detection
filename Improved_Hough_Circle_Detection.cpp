...
int main(int argc, char **argv) {
	...
	// Mat img = imread("/home/wanghan/Downloads/test5.jpg");
	if (img.data == nullptr) { //数据不存在,可能是文件不存在
		cerr << "文件" << argv[1] << "不存在." << endl;
		return 0;
	}
	namedWindow("Original Image", 0);
	cvResizeWindow("Original Image", 1200, 800); 
	...
	namedWindow("Circle Detection Display", 0);
	cvResizeWindow("Circle Detection Display", 1200, 800); 
	imshow("Circle Detection Display", img);
	namedWindow("edge", 0);
