...
int getCentre(Mat edge, Point *pp);
int* getR(int cnum, Mat& edge, Point *pp);
...
int main(){
	...
	while (true)
	{
        ...
        cvtColor(frame,edge, CV_BGR2GRAY);//转化边缘检测后的图为灰度图
        GaussianBlur( edge, edge, Size(9, 9), 2, 2 );
        ...
        //获得圆心的数目
        int c_num = getCentre(EDGE, pp);
        //得到圆的半径
        cout << c_num << endl;
        int *rad = getR(c_num, EDGE, pp);

        for (int i = 0; i < c_num; i++) {
            if (rad[i] != 0) {
                circle(frame, pp[i], rad[i], Scalar(0, 0, 255), 2, 8);
                circle(edge, pp[i], 1, 255, 2, 8);
            }
        }
        imshow("Circle Detection Display", frame);
        waitKey(100);
        delete[]rad;
        delete[]pp;
    }
